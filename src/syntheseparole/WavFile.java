...
public class WavFile
{
	...
	public static WavFile cat(String filename1, String filename2) throws IOException, WavFileException{
		...
		/* Initialisation de la lecture des sons objets FileInputStream */
		
		File songfile1 = new File(filename1);
		File songfile2 = new File(filename2);
		WavFile readWavFile1 = openWavFile(songfile1);
		WavFile readWavFile2 = openWavFile(songfile2);
		...
		int numFrames1 = (int) (readWavFile1.getNumFrames());	
		int numFrames2 = (int) (readWavFile2.getNumFrames());
		
		
		double buffer1 [] = new double[numFrames1*2] ;
		double buffer2 [] = new double[numFrames2*2] ;
		readWavFile1.readFrames(buffer1,numFrames1);
		readWavFile2.readFrames(buffer2,numFrames2);
		double bigbuffer[] = new double [2*(numFrames1 + numFrames2)];
		
		/* Initialisation de la fenetre de Hann */
		
		double hannwindow [] = new double [windowp];
		for (int j=0; j<windowp;j++) {	
			hannwindow[j] = 0.5 -0.5*Math.cos(2*j*Math.PI/windowp);
		}
		
		/* Copie des tableaux buffer1 et buffer2 dans des tableaux intermédiaires */
		
		double buffer1_pond [] = (double []) buffer1.clone();
		double buffer2_pond [] = (double []) buffer2.clone();
		
		/* Ponderation des buffer 1&2 par la fenetre de Hann */
		
		for (int k=0; k<(windowp/2);k++){
			
			buffer1_pond[2*numFrames1 - windowp/2 + k] = (double) buffer1_pond[2*numFrames1 - windowp/2 + k] * hannwindow[windowp/2 +k];
		}
		
		for (int k=0; k<(windowp/2);k++){
			
			buffer2_pond[k] = (double) buffer2_pond[k] * hannwindow[k];
		}
		
	    /* Insertion de buffer1 dans bigbuffer */
		
		for (int l=0; l<2*numFrames1;l++) {
			
			bigbuffer[l] = bigbuffer[l] + buffer1_pond[l];
		}
		
		/* Création d'indexs pour la transition et l'insertion de buffer2 */
		
		int index1 = 2*numFrames1 - windowp/2 + 1;
		int index2 = 2*(numFrames1 + numFrames2) - windowp/2;
		
		/* Remplissage de buffer2 dans bigbuffer */
		
		for (int i = index1; i< (index2+1) ; i++ ) {
			
			bigbuffer[i] = bigbuffer[i] + buffer2_pond[i-index1];
		}
